...
17–21, 2023, Seattle, WA, USA. ACM, New York, NY, USA, 12 pages. https:
...
the smart contract’s state. Auditing means ensuring a smart con-
...
ISSTA ’23, July 17–21, 2023, Seattle, WA, USA Chaofan Shou, Shangyin Tan, and Koushik Sen
all interesting snapshots into a state corpus, ItyFuzz can “time
travel” to previous states with constant complexity (𝑂 (1)). Time
...
• We present a novel snapshot-based fuzzing algorithm to re-
duce re-execution overhead for stateful smart contract fuzzing
(Section 4.2).
• We create new waypoint mechanisms optimized for prioritizing
...
a set of initial inputs, where in each iteration, an input 𝑖 from the
corpus is selected. 𝑖 is then randomly mutated to produce several
mutated inputs. The fuzzer then executes each mutated input 𝑖𝑚
...
adds the mutated input 𝑖𝑚 to the corpus I and updates the coverage
information. The addition of 𝑖𝑚 to the corpus ensures that the input
...
5 i ← random.choice(I)
6 I ← I \ {i}
7 for 𝑖𝑚 in Mutation(i):
8 f ← execute(𝑖𝑚 )
9 if f increases coverage:
10 I ← I ∪ t
11 coverage ← coverage ∪ f
...
points—some other common waypoints include execution time,
...
ItyFuzz: Snapshot-Based Fuzzer for Smart Contract ISSTA ’23, July 17–21, 2023, Seattle, WA, USA
...
ItyFuzz is that we can only memoize a set of “interesting states”,
...
re-execution. The “interestingness” of a state is defined using two
...
the trace collected during the execution is “interesting". An input is
...
In smart contract executions, a state 𝑠 can be constructed by exe-
...
form (𝑠, 𝑡), where 𝑠 ∈ 𝑆 and 𝑡 ∈ 𝑇 . Specifically, given an execution
EVM (𝑠, 𝑡) ↦ → 𝑠
′
, where 𝑠
′ 
∈ 𝑆 is the new state after execution of 𝑡 on
𝑠, the pair (𝑠, 𝑡) is added to the corpus when the combination of feed-
...
call the “infant state corpus”, say C𝑠 . Specifically, given an execution
EVM (𝑠, 𝑡) ↦ → 𝑠
′
, the state 𝑠
′ 
...
of C—they determine whether the current execution is interesting,
...
and Section 4.4.
The pseudo-code in Figure 6 summarizes ItyFuzz’s snapshot-
...
fuzzer then either mutates the transaction 𝑡 structurally to 𝑡mut or
replaces the state 𝑠 with a state, say 𝑠mut , stored in the infant state
corpus C𝑠 . The new mutant is always sound (i.e. reachable in the
...
saves the state and transaction pair (𝑠mut, 𝑡mut ) to corpus C if the
observed waypoints are interesting. ItyFuzz adds the the resulting
state 𝑠
′ 
to the infant state corpus C𝑠 if 𝑠
′ 
...
1 C ← Corpus() # transaction and state pair corpus
2 C𝑠 ← Corpus() # infant state corpus
3
4 while true:
5 (𝑠,𝑡) ← Next( C)
6 if Random(0, 1) > P:
7 𝑡mut ← Mutation(𝑡) # mutate transaction
8 𝑠mut ← s
9 else
10 𝑠mut ← Next( C𝑠 ) # mutate state by fetching from infant
state corpus
11 𝑡mut ← t
12
13 # execute transaction 𝑡mut on state 𝑠mut
14 # 𝑓 represents waypoints, 𝑠
′ 
is the resulting state
15 𝑓 , 𝑠
′ 
←EVM(𝑠mut , 𝑡mut )
16 if 𝑊(𝑠,𝑡 ) (f) is interesting: # get feedback for waypoints of
execution
17 C ← C ∪ (𝑠mut , 𝑡mut )
18 if 𝑊(𝑠 ) (f, 𝑠
′
...
instruction’s “interestingness” at present is impossible. To resolve
...
3 𝐿 (Loc % MAP_SIZE) ← true ;
4 if Current Instruction is store(Loc, Value) then
5 𝑆(Loc % MAP_SIZE)(BucketOf(Value)) ← true;
...
true */
1 if ∃Bucket ∈ 𝑆 s.t. Bucket changes from false to true then
...
ISSTA ’23, July 17–21, 2023, Seattle, WA, USA Chaofan Shou, Shangyin Tan, and Koushik Sen
or not happens as follows. Load map 𝐿 maps an abstract memory
location to a Boolean value initialized to false. When a load instruc-
tion Load(Loc) is executed, 𝐿(Loc % MAP_SIZE) is set to true. Loc %
...
lower storage and lookup overhead. Store map 𝑆 maps each abstract
...
abstracted to determine its bucket. If 𝑏 ∈ 𝐵 is the bucket abstraction
of Value, 𝑆 (Loc % MAP_SIZE)(𝑏) is set to true. After execution, if
a bucket in the store map 𝑆 changes from false to true, and the
corresponding location in 𝐿(Loc % MAP_SIZE) is also true (line 2 at
...
would be evaluated to be interesting, increasing the size of C𝑠 . As
size of C𝑠 increases, it becomes harder to effectively choose the
...
execution 𝐶local with maximum possible value (line 1). Then during
...
smaller distance in the local map (𝐶local) compared to the map that
records the smallest distance ever recorded in previous executions
(𝐶), the execution is considered interesting.
...
waypoint, ItyFuzz simply prune states with less “interestingness”
...
been chosen and executed. Then, ItyFuzz discards 𝑀 states with
lowest votes/visits and have visits greater than a threshold 𝑂.
If a state can not minimize comparison map over 𝑂 visits, then it is
...
as the EVM executor for convenience. We also leverage revm’s inter-
...
of ItyFuzz:
• ItyFuzz- Rand: snapshots states and stores them into the infant
state corpus with a likelihood of 50%.
• ItyFuzz- DF: snapshots states and stores them into the infant
...
showcasing our tool’s effectiveness. There is no false positive as
...
in the store map’s bucket to true. After that, states are less likely
...
balance 𝑘 of tokens, approve contract with 𝑘
′ 
token, deposit into
the contract through calling lockToken with the correct amount
of ETH sent for locking fee and 𝑣, such that 𝑣 < 𝑘
′ 
∧ 𝑣 < 𝑘, and
...
Felty (Eds.). ACM, 66–77. https://doi.org/10.1145/3167084
[2] bluealloy. 2022. revm - Rust Ethereum Virtual Machine. https://github.com/
