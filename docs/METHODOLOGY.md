...
Total: 5 slots = 5 Ã, SSTORE = 100,000 gas (cold)
```
...
Total: 2 slots = 2 Ã, SSTORE = 40,000 gas (cold)
...
x = amountIn (frontrun)
y = (reserve_token Ã, x) / (reserve_ETH + x) Ã, (reserve_ETH) / (reserve_token + Î”v)

Î”v = victim.amountIn (transaksi korban)
```
...
Penalty = Victim.amountOut Ã, (Î» Ã, P_detect / 100,000,000)

Dimana:
Î» = 15000 (faktor penalti 1.5x)
...
```
E[Profit_a] = P(not detected) Ã, Profit_a + P(detected) Ã, Profit_a'

P(not detected) = 1 - P_detect/10000 = 0.04 (4%)
P(detected) = P_detect/10000 = 0.96 (96%)

E[Profit_a] = 0.04 Ã, Profit_a + 0.96 Ã, (Profit_a - Penalty)
            = Profit_a - 0.96 Ã, Penalty
```

**Kondisi Profitable:**
```
Profit_a > Penalty
(Î”vÂ² Ã, x) / ((reserve_ETH + x)Â² Ã, reserve_ETH) > Î”v Ã, (Î» Ã, P_detect / 100,000,000)

Jika Î» = 1.5 dan P_detect = 0.96:
Profit_a > Î”v Ã, 1.44 Ã, 10â»âµ
```
...
```
C_static = Î£ (batch_bytes Ã, 16 Ã, L1_fee)

Dimana:
batch_bytes = tx_count Ã, tx_size (tanpa kompresi)
...
### 3.3 Compression Factor (Î±)

```
Î± = 1 - compression_ratio

Untuk rollup:
Î±_RLP = 0.85 (RLP encoding, 15% savings)
Î±_ZK  = 0.70 (ZK proof compression, 30% savings)
Î±_combined = 0.88 (combined, 12% savings) â† Digunakan dalam simulasi
```
...
```
Trigger WHEN:
  beff_bytes â‰¥ TARGET_BATCH_BYTES (100 KB)
  OR blocks_since_last_batch â‰¥ MAX_DELAY (25 blok)

Dimana:
beff_bytes = pending_txs Ã, TX_SIZE Ã, Î±
```
...
Penalty(amount, anomalyScore) = min(
  amount Ã, Î» Ã, anomalyScore / 100,000,000,
  amount
)

Î» = 15000 (faktor risiko)
anomalyScore = 0 - 10000 (skor deteksi)
```

### 4.2 Penalty Analysis

| anomalyScore | Î» | Penalty Rate | Penalty (100 ETH) |
|--------------|---|--------------|---------------------|
| 0 | 15000 | 0% | 0 ETH |
...
U(a) = P(undetected) Ã, Profit - P(detected) Ã, Penalty
...
**Hâ‚€ (Null Hypothesis):** Tidak ada perbedaan signifikan gas cost antara bridge statis dan bridge dinamis.

**Hâ‚ (Alternative):** Bridge dinamis (EWS + EIP-1153) menghemat gas secara signifikan dibanding bridge statis.

**Test:** Paired t-test (n â‰¥ 100 sampel)

```
t = (xÌ„_static - xÌ„_dynamic) / (s_d / âˆšn)

Dimana:
xÌ„_static = rata-rata gas cost bridge statis
xÌ„_dynamic = rata-rata gas cost bridge dinamis
...
```
CI_95% = (xÌ„_diff - t_Î±/2 Ã, s_d/âˆšn, xÌ„_diff + t_Î±/2 Ã, s_d/âˆšn)
```

### 5.3 Effect Size (Cohen's d)

```
d = (xÌ„_static - xÌ„_dynamic) / s_pooled
...
Ideal: MPS â‰¥ 0.96 (96% detection rate)
```
