...
| Difficulty | Sedang |
| Status | ‚úÖ SELESAI (02 Jun 2026) |
| Compile | ‚è≥ Perlu Foundry (forge-std dependency) |
...
Expected Utility < 0
‚Üí 0.04 √, Profit < 0.96 √, Penalty
‚Üí Profit < 24 √, Penalty
```
...
function testEconomicDeterrence_AttackerLoses() public {
    ...
    //    Dalam sandwich: Profit ‚âà (Œîv¬≤ √, x) / ((reserve_ETH + x)¬≤ √, reserve_ETH)
    //    Simplified: Profit ‚âà 1-3% dari attack volume
 ...
 *      Ini adalah BASELINE ‚Äî bukti bahwa tanpa pertahanan, attacker untung.
 */
function testEconomicDeterrence_Baseline_NoEWS() public {
    ...
    // Profit sandwich ‚âà 1-3% dari attack volume
...
|--------|-------------|---------------|------------------|-------------|
| 10 ETH | 0.2 ETH | 1.44 ETH | -1.36 ETH | ‚ùå Tidak |
| 50 ETH | 1.0 ETH | 7.2 ETH | -6.88 ETH | ‚ùå Tidak |
| 100 ETH | 2.0 ETH | 14.4 ETH | -13.76 ETH | ‚ùå Tidak |

**Kesimpulan:** Dengan P_detect=96% dan Œª=1.5, sandwich attack **selalu tidak profitable** untuk semua volume.
...
- [ ] Jalankan `forge test --match-test "EconomicDeterrence" -vvv` ‚Äî perlu Foundry
- [ ] Catat data untuk tabel di paper ‚Äî perlu Foundry
