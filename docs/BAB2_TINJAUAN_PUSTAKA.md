...
Attacker.call(withdraw(amount)) 
  â†’ Bridge.transfer(attacker, amount) 
    â†’ attacker.receive() 
      â†’ Attacker.call(withdraw(amount))  [RECURSIVE CALL]
```
...
Dalam konteks *bridge blockchain*, *reentrancy attack* menjadi ancaman yang sangat serius karena *bridge* menyimpan aset dalam jumlah besar dari banyak pengguna. Zheng et al. (2023) dalam studi skala besar terhadap 230.548 kontrak terverifikasi di Etherscan menemukan bahwa 21.212 kontrak memiliki masalah *reentrancy*â€”angka yang menunjukkan betapa merajalelanya kerentanan ini.
...
```
Î”G_packing = (N_before - N_after) Ã, SSTORE_cold
            = (2 - 1) Ã, 20.000
...
Solidity 0.8.x secara default menerapkan pemeriksaan *overflow/underflow* pada setiap operasi aritmatika. Meskipun ini meningkatkan keamanan, pemeriksaan tersebut menambah biaya gas sekitar 20-50 gas per operasi. Pada situasi di mana *overflow* dapat dipastikan tidak terjadi â€” misalnya setelah validasi input yang memastikan nilai positif â€” blok `unchecked` dapat digunakan untuk menghilangkan pemeriksaan ini (Solidity Documentation, 2023).
...
**Penghematan:**
```
Î”G_tstore = G_mutex - G_tstore
          = 22.900 - 200
          = 22.700 gas (99.1% savings)
```

Penghematan ini menjadi sangat berarti untuk *bridge* dengan volume transaksi tinggi. Pada 1.000 transaksi per hari, penghematan kumulatif mencapai 22,7 juta gasâ€”setara sekitar 0,0227 ETH pada harga gas 10 Gwei (Paradigm, 2023). Untuk *upgradeable smart contracts* yang memiliki interaksi *storage* intensif, angka ini tentu tidak bisa diabaikan.
...
```
Profit_a â‰ˆ (Î”vÂ² Ã, x) / ((reserve_ETH + x)Â² Ã, reserve_ETH)
```

Di mana `Î”v` adalah jumlah transaksi korban dan `x` adalah jumlah *frontrun* penyerang.
...
2. **Analisis pola**: Ketika transaksi baru terdeteksi, EWS memeriksa apakah terdapat pola Ta1â†’Tv yang mencurigakan, di mana Ta1 merupakan *frontrun* dan Tv merupakan transaksi *victim*.

3. **Penilaian skor anomali**: Jika pola terdeteksi, EWS menghitung skor anomali berdasarkan faktor risiko (Î») dan probabilitas deteksi (P_detect).
...
Model penalti ekonomi pada EWS dibangun di atas prinsip *incentive compatibility*â€”membuat serangan menjadi tidak menguntungkan secara ekonomi. Formula penalti didefinisikan sebagai:
...
- `Î»` = faktor penalti risiko (*default*: 15.000 = 1,5x)
...
```
E[Profit_a] = P(not detected) Ã, Profit - P(detected) Ã, Penalty
            = 0,04 Ã, Profit - 0,96 Ã, Penalty
```

Kondisi agar serangan tidak menguntungkan:
```
U(a) > 0
Profit > 24 Ã, Penalty (untuk P_detect = 96%)
```
...
C_calldata = beff_bytes Ã, 16 Ã, L1_fee
C_blob = BLOB_GAS_SIZE Ã, blob_fee

beff_bytes = tx_count Ã, tx_size Ã, Î± (compression factor)
BLOB_GAS_SIZE = 131.072 gas (128 KB)
```

Di mana `Î±` (*compression factor*) merupakan rasio ukuran data terkompresi terhadap data asli. Untuk *rollup* dengan kompresi RLP, Î± â‰ˆ 0,85; untuk ZK *proof* compression, Î± â‰ˆ 0,70; dan untuk kompresi gabungan, Î± â‰ˆ 0,88.
