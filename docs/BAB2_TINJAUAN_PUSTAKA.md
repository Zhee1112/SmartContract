...
Model ini relevan dengan penelitian karena setiap instruksi yang dieksekusi memiliki kontribusi biaya gas yang berbeda. Optimasi statis berfokus pada pengurangan jumlah instruksi dan penggunaan slot storage yang lebih efisien, sedangkan optimasi dinamis memanfaatkan fitur-fitur baru EVM seperti EIP-1153 untuk mengurangi biaya operasi yang berulang.
...
Celah keamanan ini terjadi karena pelanggaran pola Checks-Effects-Interactions (CEI), di mana interaksi eksternal (transfer ETH) dilakukan sebelum efek (pembaruan state) diterapkan (ConsenSys, 2023). Dalam konteks bridge blockchain, reentrancy attack merupakan ancaman yang sangat serius karena bridge menyimpan aset dalam jumlah besar dari banyak pengguna.

Data dari Chainalysis (2022) menunjukkan bahwa reentrancy attack menjadi pola serangan paling dominan dalam eksploitasi bridge, dengan kerugian kumulatif mencapai miliaran dolar. Ronin Bridge (2022) mengalami kerugian $620 juta akibat reentrancy pada validator set withdrawal (Trail of Bits, 2022), sementara Wormhole Bridge kehilangan $320 juta akibat bypass verifikasi signature (Rekt.news, 2024).
...
Namun, CEI memiliki keterbatasan. Pola ini bersifat statis dan tidak dapat mendeteksi pola serangan yang lebih kompleks seperti MEV sandwich attack, yang memerlukan mekanisme deteksi dinamis (Tsankov et al., 2018).

### 2.2.3 Studi Kasus Eksploitasi Bridge

Analisis terhadap eksploitasi bridge dalam tiga tahun terakhir mengungkapkan pola serangan yang berulang dan mengkhawatirkan:

**Ronin Bridge (Maret 2022)**: Eksploitasi ini melibatkan kompromi terhadap private key validator, namun juga mengungkapkan kelemahan fundamental dalam mekanisme validasi transaksi bridge. Kerugian mencapai $620 juta dalam bentuk ETH dan USDC (Chainalysis, 2022).
...
**Nomad Bridge (Agustus 2022)**: Eksploitasi ini terjadi akibat kesalahan konfigurasi pembaruan kontrak, yang memungkinkan siapa saja memverifikasi transaksi palsu. Kerugian mencapai $190 juta (Rekt.news, 2024).
...
Teknik ini diterapkan pada `BridgeStaticOnly` dan `VictimBridge`, yang menghasilkan penghematan deployment gas sebesar 11.447 gas dibandingkan `UnoptimizedBridge` yang menggunakan variabel terpisah (Austin Griffith, 2023).
...
Sebelum Solidity 0.8.4, mekanisme revert standar menggunakan string error message, yang memerlukan encoding ABI dan menambah biaya gas sekitar 200-500 gas per revert. Custom errors, yang diperkenalkan melalui EIP-4844 Discussion, memungkinkan deklarasi error tanpa string, sehingga mengurangi biaya revert sekitar 50 gas per permohonan (Solidity 0.8.4 Release Notes, 2021).

Pada kontrak `BridgeStaticOnly`, `VictimBridge`, dan `LightweightBridge`, seluruh pesan error digantikan dengan custom error, yang menghasilkan penghematan kumulatif yang signifikan terutama pada jalur error yang sering dieksekusi.
...
EIP-1153 (Transient Storage Opcodes) merupakan Ethereum Improvement Proposal yang memperkenalkan dua opcode baru: TSTORE (Transient Store) dan TLOAD (Transient Load). Proposal ini diaktifkan melalui fork Cancun pada Maret 2024, bersamaan dengan EIP-4844 dan beberapa peningkatan lainnya (EIP-1153, 2021).
...
Penghematan ini bersifat eksponensial untuk bridge dengan volume transaksi tinggi. Pada 1.000 transaksi per hari, penghematan kumulatif mencapai 22,7 juta gas, yang setara dengan sekitar 0,0227 ETH pada harga gas 10 Gwei (Paradigm, 2023).
...
EIP-4844 (Shard Blob Transactions), yang diaktifkan melalui fork Cancun pada Maret 2024, memperkenalkan jenis transaksi baru yang membawa "blob" — data sementara yang dapat diakses oleh klien light tetapi tidak disimpan secara permanen dalam state Ethereum (EIP-4844, 2023). Blob dirancang khusus untuk kebutuhan rollup, yang sebelumnya harus mengirimkan data transaksi sebagai calldata konvensional dengan biaya yang jauh lebih tinggi.
...
Ethereum Foundation (2022) mendefinisikan rollups berdasarkan dua karakteristik kunci: (1) data transaksi harus tersedia di L1 untuk memungkinkan verifikasi independen, dan (2) state transition harus dapat divalidasi secara cryptographi atau melalui mekanisme challenge period.
...
Pada penelitian ini, EWS diimplementasikan melalui kontrak `MonitorMock` yang menggunakan EIP-1153 transient storage untuk melacak status transaksi, mendeteksi pola sandwich attack, dan menerapkan penalti ekonomi secara otomatis.
...
1. **Optimasi Gas Statis** (Bagian 2.3): Variable packing, CEI pattern, unchecked arithmetic, custom errors, calldata parameter, dan immutable variables.

2. **EIP-1153 Transient Storage** (Bagian 2.4): Mekanisme TSTORE/TLOAD untuk reentrancy guard yang efisien dan pelacakan status transaksi.

3. **MEV Protection On-Chain** (Bagian 2.5): Deteksi pola sandwich attack dan penerapan penalti ekonomi melalui EWS.

4. **Dynamic Rollup Submission** (Bagian 2.9): Routing dynamic antara blob dan calldata berdasarkan harga gas real-time.

5. **Arsitektur Bridge 4-Tier** (Bagian 2.10): Hierarki optimasi dari baseline (Tier A) hingga lightweight dynamic (Tier D).
...
[1] Buterin, V. (2014). "Ethereum Whitepaper." Ethereum Foundation.
...
[5] OpenZeppelin. "ReentrancyGuard Documentation." OpenZeppelin Contracts.

[6] SWC Registry. "SWC-107: Reentrancy."

[7] ConsenSys. "Smart Contract Best Practices: Checks-Effects-Interactions."

[8] Ethereum StackExchange. "Storage Layout in Solidity."

[9] Austin Griffith. "Solidity Gas Optimization Techniques."

[10] Solidity Documentation. "Unchecked Blocks."

[11] EIP-1283. "Net Gas Metering for SSTORE."

[12] Solidity 0.8.4 Release Notes. "Custom Errors Feature."

[13] EIP-1967. "Standard Proxy Storage Slots."

[14] EIP-1153. "Transient Storage Opcodes."

[15] EIP-1153 Rationale. "Design Rationale for Transient Storage."

[16] OpenZeppelin. "TransientStorageGuard."

[17] Solady. "TransientStorageLock."

[18] Paradigm. (2023). "EIP-1153: Transient Storage Deep Dive."

[19] Ethereum Research. "Transient Storage Gas Benchmarks."

[20] OpenZeppelin. (2024). "Transient Storage Security Model."

[21] Trail of Bits. (2024). "EIP-1153 Security Assessment."

[22] Daian, P. et al. (2020). "Flash Boys 2.0: Frontrunning in Decentralized Exchanges." IEEE Symposium on Security and Privacy.

[23] Flashbots. (2021). "MEV Research."

[24] Qin, K. et al. (2021). "Quantifying Blockchain Extractable Value."

[25] Barbereau, T. et al. (2023). "Decentralised Finance's Unregulated Market Structure."

[26] Flashbots. "MEV-Share: MEV Redistribution."

[27] CoW Protocol. "Batch Auctions for MEV Protection."

[28] Uniswap V3. "TWAP Oracle."

[29] EIP-4844. "Shard Blob Transactions."

[30] EIP-4844 Rationale. "Design Rationale for Blob Transactions."

[31] Ethereum Foundation. "Blob Fee Market."

[32] Dune Analytics. "Blob Usage Dashboard."

[33] Optimism. "Blob Integration Guide."

[34] Arbitrum. "Blob Support in Nitro."

[35] Ethereum Foundation. "Rollups Documentation."

[36] Buterin, V. (2021). "An Incomplete Guide to Rollups."

[37] Optimism. "Optimism Architecture."

[38] Arbitrum. "Arbitrum Technology."

[39] StarkWare. "StarkNet Documentation."

[40] zkSync. "zkSync Era Architecture."

[41] Chainalysis. (2022). "Ronin Bridge Exploit Analysis."

[42] Trail of Bits. (2022). "Wormhole Bridge Security Report."

[43] Rekt.news. "Top DeFi Hacks 2020-2024."

[44] Hop Protocol. "Technical Documentation."

[45] LayerZero. "Stargate Finance Documentation."

[46] Adams, H. et al. (2021). "Uniswap V3 Core."

[47] Foundry Documentation. "Invariant Testing Guide."

[48] Trail of Bits. "How to Write Invariant Tests."

[49] Certora. "Formal Verification of Solidity."

[50] Chen, J. et al. (2022). "Gas Optimization in Smart Contracts."

[51] Zhang, Y. et al. (2023). "A Survey on Smart Contract Gas Optimization."

[52] Schwartz, A. et al. (2023). "The MEV Problem: A Survey."

[53] Albert, E. et al. (2019). "Running on Fumes: Preventing Out-of-Gas Vulnerabilities."

[54] Jiang, B. et al. (2018). "Sereum: Protecting Existing Smart Contracts Against Re-Entrancy Attacks."

[55] Tsankov, P. et al. (2018). "Securify: Practical Security Analysis of Smart Contracts."

[56] Grech, N. et al. (2020). "MadMax: Surviving Out-of-Gas Situations in Solidity."

[57] Nguyen, T. et al. (2020). "SFuzz: An Efficient Adaptive Fuzzer for Solidity Smart Contracts."

[58] Lagouvardos, S. et al. (2020). "Precise Static Modeling of Ethereum Memory."

[59] So, S. et al. (2019). "DeFiRanger: Detecting Price Manipulation Attacks on DeFi Protocols."

[60] Tan, S. et al. (2022). "Large Language Models for Smart Contract Vulnerability Detection."

[61] Feng, X. et al. (2019). "Bug Searching in Smart Contract."

[62] Liang, X. et al. (2019). "Smart Contract Repair."
